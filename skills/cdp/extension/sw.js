...
chrome.debugger.onDetach.addListener((source, reason) => {
  ...
  if (sessionId) {
    emit({ method: 'Inspector.detached', params: { reason: reason || 'target_closed' }, sessionId });
    emit({ method: 'Target.detachedFromTarget', params: { sessionId, targetId: String(source.tabId) } });
  }
});
...
async function attachToTarget(params) {
  ...
  if (!Number.isFinite(tabId)) throw new Error('Target.attachToTarget: unknown targetId ' + targetId);
