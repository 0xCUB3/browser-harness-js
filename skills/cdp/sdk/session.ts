...
export class Session implements Transport {
  ...
  private async _connect(opts: ConnectOptions = {}): Promise<void> {
    ...
    if (browsers.length === 0) {
      ...
      throw new Error(
        `No running browser with remote debugging detected. Load this repo's unpacked extension/ side-panel relay, enable remote debugging from chrome://inspect > "Discover network targets", or pass { profileDir } / { wsUrl } explicitly. Scanned: ${scanned}.`,
      );
    }
    const errors: string[] = [];
...
async function detectExtensionRelay(): Promise<string | undefined> {
  ...
  try {
    // A daemon can outlive its extension. Do not treat its always-present /cdp
    // endpoint as usable unless /health confirms that a relay is attached.
    const healthResponse = await fetch(`http://127.0.0.1:${port}/health`, {
      signal: AbortSignal.timeout(300),
    });
    if (!healthResponse.ok) return undefined;
    const health: any = await healthResponse.json();
    if (health.extension !== true && health.extensionConnected !== true) return undefined;
